    ...
    <div className="relative mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
      ...
      <nav aria-label="Primary navigation" className="hidden items-center gap-x-5 gap-y-2 text-sm font-medium lg:flex">
        ...
        <div className="nav-group nav-group-separated">
          {!loading && (user ? <><Link href="/profile" onClick={() => requestPageGlint("/profile")} className={linkClass("/profile")}>Profile</Link><button type="button" onClick={() => { requestPageGlint(); void signOut().then(() => router.push("/")); }} className={linkClass("/sign-out")}>Sign out</button></> : <><Link href="/login" onClick={() => requestPageGlint("/login")} className={linkClass("/login")}>Log in</Link><Link href="/sign-up" onClick={() => requestPageGlint("/sign-up")} className="rounded-md bg-emerald-600 px-3 py-2 text-white">Sign up</Link></>)}
          <ThemeSettings />
        </div>
      </nav>
