...
export function HomeLearningStatus() {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-label="Learning status">
    ...
    <GuestAccessLink href={status.completedToday ? "/progress" : "/daily"} className="mt-4 inline-flex min-h-11 items-center justify-center rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">{status.completedToday ? "View progress" : "Start Daily Practice"}</GuestAccessLink>
  </section>;
}
